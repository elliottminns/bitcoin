Livecoin Core version *0.15.1* is now available from:
...
installer (on Windows) or just copy over `/Applications/Livecoin-Qt` (on Mac)
...
A number of changes to the way Livecoin Core deals with peer connections and invalid blocks
